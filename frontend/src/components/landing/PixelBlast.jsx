...
const PixelBlast = ({
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      ...
      const animate = () => {
        ...
        if (composer) {
          ...
          composer.passes.forEach(p => {
            p.effects?.forEach(eff => { const u = eff.uniforms?.get('uTime'); if (u) u.value = uniforms.uTime.value; });
          });
          composer.render();
      ...
      if (t.liquidEffect) {
        const uStr = t.liquidEffect.uniforms.get('uStrength'); if (uStr) uStr.value = liquidStrength;
        const uFreq = t.liquidEffect.uniforms.get('uFreq');    if (uFreq) uFreq.value = liquidWobbleSpeed;
      }
